...
namespace szabiyako
{
	...
	public:
		Matrix()
		{
			...
			n = 0;
			epsilon = 2;
			std::cout << "Vvedite razmer matricy:\n";
			std::cin >> m;
			std::cout << "x\n";
			std::cin >> n;
			while ((m < 1) || (n < 1))
			{
				std::cout << "Vvedite KORREKTNYJ razmer matricy:\n";
				std::cin >> m;
				std::cout << "x\n";
				std::cin >> n;
			}
		...
		void G_show()
		{
			...
			//l[i] teper' dlina s tochkoj i znakom (esli oni est')
			for (int i = 0; i < m; i++)
			{
				std::cout << '|';
				for (int j = 0; j < n; j++)
		...
		void G_changeElement()
		{
			...
			T c;
			G_show();
			std::cout << "\nVvedite nomer stroki jelementa:";
			std::cin >> a;
			if ((a - 1 < m) && (a > 0))
			{
				std::cout << "Vvedite nomer stolbca jelementa:";
				std::cin >> b;
				if ((b - 1 < n) && (b > 0))
				{
					std::cout << "Vvedite novoe znachenie jelementa:";
					std::cin >> c;
					inside[a - 1][b - 1] = c;
					G_show();
				}
				else std::cout << "\nOshibka, vyhod za granicy matricy\n";
			}
			else std::cout << "\nOshibka, vyhod za granicy matricy\n";
		}
		void G_rewrite()
		...
		void G_debug()
		{
			...
			std::cout << "\n N=" << returnN();
			if (m == n)std::cout << "\n\n Opredelitel' = " << det();
			else std::cout << "\n\n Opredelitel' najti nel'zja tak kak chislo strok i stolbcov ne sovpadaet";
			std::cout << "\n\n V vide boolean:\n";
			G_boolShow();
			std::cout << "\n V transponirovannom vide:";
			G_showTransp();
			/*std::cout << "\n Reshjonnaja metodom Zhordana-Gaussa:";
			Matrix <T> s(1,1);
			s.G_rewrite();
			G_gaus(s);
			*/
		...
		void G_gaus(Matrix <T> s)
		{
			if ((s.returnM() == m) && s.returnN() == 1)
			{
				...
				if (m >= n)
				{
					...
					answer.G_show();
					//debug!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!PROBLEMA ZDES''(+ posle else tozhe nado pomenjat'!!!)
					s.G_show();
					for (int q = o - 1; q > 0; q--)
					{
						for (int i = m - 1; i >= 0; i--)
				...
				std::cout << "\nMatrica neizvestnyh:";
				answer.G_show();
				std::cout << "\nMatrica svobodnyh chlenov:";
				for (int i = 0; i < m; i++)if (std::abs(s.getFromZero(i, 0)) == 0) s.setFromZero(i, 0, 0);
				s.G_show();
			}
			else std::cout << "\nOshibka! matrica svobodnyh chlenov ne sovpadaet po razmeram s matricej neizvestnyh";
		}
	};
